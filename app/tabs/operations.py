...
def render_operations_tab(tab, ctx: dict, default, description) -> None:
    with tab:
        ...
        with left_building:
            ...
            g1, g2 = st.columns(2)
            with g1:
                grans_utilities = st.number_input("Electricity / internet", min_value=0, value=default("operations", "grans_utilities", 350_000), step=10_000, help=description("operations", "grans_utilities", "Projected Gransdalen cost for electricity, heating, internet, and similar utilities. Increasing it raises operating pressure."))
                grans_municipal = st.number_input("Municipal / water / sewage", min_value=0, value=default("operations", "grans_municipal", 120_000), step=10_000, help=description("operations", "grans_municipal", "Projected Gransdalen cost for municipal charges, water, and sewage. Increasing it raises annual building carrying cost."))
                grans_accounting = st.number_input("Auditor / accounting", min_value=0, value=default("operations", "grans_accounting", 100_000), step=10_000, help=description("operations", "grans_accounting", "Projected Gransdalen accounting, audit, or finance administration cost."))
                grans_insurance = st.number_input("Building insurance", min_value=0, value=default("operations", "grans_insurance", 180_000), step=10_000, help=description("operations", "grans_insurance", "Projected building-insurance cost for Gransdalen."))
                grans_security = st.number_input("Security / control", min_value=0, value=default("operations", "grans_security", 40_000), step=10_000, help=description("operations", "grans_security", "Projected Gransdalen cost for security, alarms, and basic safety/control systems."))
            with g2:
                grans_maintenance = st.number_input("Minor maintenance", min_value=0, value=default("operations", "grans_maintenance", 150_000), step=10_000, help=description("operations", "grans_maintenance", "Projected Gransdalen cost for minor repairs, replacement, and upkeep."))
                grans_cleaning = st.number_input("Cleaning / consumables", min_value=0, value=default("operations", "grans_cleaning", 70_000), step=10_000, help=description("operations", "grans_cleaning", "Projected Gransdalen cost for cleaning supplies and routine consumables."))
                grans_caretaker = st.number_input("Caretaker / snow / outdoor", min_value=0, value=default("operations", "grans_caretaker", 180_000), step=10_000, help=description("operations", "grans_caretaker", "Projected Gransdalen cost for caretaker, snow clearing, and outdoor maintenance."))
                grans_other = st.number_input("Other building costs", min_value=0, value=default("operations", "grans_other", 100_000), step=10_000, help=description("operations", "grans_other", "Any additional Gransdalen building cost not already listed."))
                grans_contingency = st.number_input("Operating contingency", min_value=0, value=default("operations", "grans_contingency", 100_000), step=10_000, help=description("operations", "grans_contingency", "Extra annual buffer for Gransdalen operating surprises. Increasing it raises prudence and cost at the same time."))
